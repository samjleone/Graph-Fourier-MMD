...
def graph_mmd(data, labels, n_neighbors=10, method="chebyshev", order=30, T=None):
    start = time.time()
    graph = graphtools.Graph(data, use_pygsp=True)
    if T is None:
        T = graph.L.trace()
    filt = pygsp.filters.Filter(
        graph, kernels=[lambda x: [i**-0.5 if i > 0 else 0 for i in x]]
    )
    embeddings = (filt.filter(labels, method=method, order=order) * (T**0.5)).T
    neigh = NearestNeighbors(
...
def graph_mmd_exact(data, labels, n_neighbors=10, method="exact", order=30, T=None):
    return graph_mmd(
        data, labels, n_neighbors=n_neighbors, method=method, order=order, T=T
    )
...
def mmd_rbf(X, Y, weightsX, weightsY, gamma=1.0):
    ...
    Returns:
        [scalar] -- [MMD value]
    """
    # TODO ignores weights for now, only applicable for point clouds
...
def pairwise_mmd(data, labels, n_neighbors=10):
    return pairwise_distribution_distance(data, labels, mmd_rbf, n_neighbors)
